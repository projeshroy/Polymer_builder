		 ...
		 for(int j = 1; j < Polymer_length; j++)
			...
			std::cout << " Given monomer_id " << monomer_id[j] << std::endl;
            	...
            	for(int j = 0; j < monomer_count; j++){
            	    ...
            	    dihedral_connection_values_matrix(monomer_id[j]-1, atom_id-1) = dihedral;
            	
            	    Monomer_atom_type_names_matrix(monomer_id[j]-1, atom_id-1) = atom_type_name;
            	    Mid_Monomer_atom_type_names_matrix(monomer_id[j]-1, atom_id-1) = atom_type_name;
            	    Ini_Monomer_atom_type_names_matrix(monomer_id[j]-1, atom_id-1) = atom_type_name;
            	    Fin_Monomer_atom_type_names_matrix(monomer_id[j]-1, atom_id-1) = atom_type_name;
     
            	    Monomer_fragment_names_matrix(monomer_id[j]-1, atom_id-1) = fragment_name;
            	    Monomer_atom_charges_matrix(monomer_id[j]-1, atom_id-1) = charge;
            	    Monomer_fragment_type_indices_matrix(monomer_id[j]-1, atom_id-1) = fragment_type_index;	
            	}		
        }		
	}
